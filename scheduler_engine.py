...
def fetch_data_from_db(year, month, weekend_dates, team_id):
    ...
    for eng_id, name, max_shifts in active_engs:
        ...
        eng_max_shifts[name] = max_shifts
        ...
        if res_avail:
            nums = [int(x.strip()) for x in res_avail[0].split(",") if x.strip()]
            availability[name] = [weekend_day_map[n] for n in nums if n in weekend_day_map]
        else:
            availability[name] = []
            
        # Fetch Leave Blockouts
        cursor.execute("SELECT block_date FROM leave_blockouts WHERE engineer_id = %s AND TO_CHAR(block_date, 'YYYY-MM') = %s", (eng_id, year_month))
...
def fetch_boundary_roster(year, month, team_id):
    ...
    for row in cursor.fetchall():
        ...
        if date_str not in boundary_roster: boundary_roster[date_str] = []
    ...
    if dates:
        ...
        for date_str, assigned_names in roster.items():
            for name in assigned_names:
                ...
                cursor.execute("INSERT INTO roster_assignments (shift_date, team_id, engineer_id) VALUES (%s, %s, %s)", (date_str, team_id, eng_id))
    conn.commit()
    cursor.close()
    conn.close()
...
def day_name(date_str): return datetime.strptime(date_str, "%Y-%m-%d").strftime("%A")
...
def count_7_day_violations(roster):
    ...
    for d in sorted(roster.keys()):
        if day_name(d) == "Sunday":
            ...
            if next_sat in roster: violations += len(set(roster[d]).intersection(set(roster[next_sat])))
    return violations
...
def evaluate_solution(solution, availability):
    ...
    unbalanced = sum(1 for e in active_engs if solution["shifts_count"][e] >= 2 and (solution["saturday_count"][e] == 0 or solution["sunday_count"][e] == 0))
...
def draft_roster(availability, eng_max_shifts, eng_leaves, weekend_dates, settings, boundary_roster, seed=42, timeout_seconds=10):
    ...
    day_avail_counts = {d: sum(1 for e in engineers if d in availability.get(e, []) and d not in eng_leaves.get(e, [])) for d in weekend_dates}
    sorted_days = sorted(weekend_dates, key=lambda d: day_avail_counts[d])
    
    roster = copy.deepcopy(boundary_roster)
    for d in weekend_dates: roster[d] = [] 
    ...
    def solve(day_idx):
        ...
        candidates = []
        for e in engineers:
            if current_day not in availability.get(e, []): continue
            if current_day in eng_leaves.get(e, []): continue
            if shifts_count[e] >= eng_max_shifts[e]: continue
...
def get_smart_suggestion(engineers, availability, eng_max_shifts, eng_leaves, weekend_dates, settings):
    """Analyzes the math to find exactly why the roster failed and suggests a fix."""
    lowest_margin = 999
    bottleneck_day = None
    needed_for_bottleneck = 0

    for d in weekend_dates:
        needed = settings['sat_coverage'] if day_name(d) == "Saturday" else settings['sun_coverage']
        # Count engineers who are available AND not on leave
        ...
        if margin < lowest_margin:
            ...
            bottleneck_day = d
            needed_for_bottleneck = needed

    if bottleneck_day:
        # Find engineers who are NOT working this day, NOT on leave, and have shift capacity
        potential_helpers = [e for e in engineers if bottleneck_day not in availability.get(e, []) and bottleneck_day not in eng_leaves.get(e, []) and eng_max_shifts.get(e, 0) > 0]
        
        if potential_helpers:
            helpers_str = ", ".join(potential_helpers[:3]) # Suggest up to 3 names
    ...
    return "💡 AI Suggestion: Try reducing the 'Engineers required per shift' in Team Settings, or increase the Max Shifts allowed for your engineers."

# ==========================================
# THE MAIN EXPOSED API / ENTRY POINT
# ==========================================
def generate_monthly_roster(year, month, team_id, seed=42):
    try:
        ...
        if not engineers: return {"success": False, "message": "No active engineers found for this team."}

        # Capacity Check
        total_needed = len([d for d in weekend_dates if day_name(d) == "Saturday"]) * settings['sat_coverage'] + len([d for d in weekend_dates if day_name(d) == "Sunday"]) * settings['sun_coverage']
        ...
        roster, avg_pref = draft_roster(availability, eng_max_shifts, eng_leaves, weekend_dates, settings, boundary_roster, seed, 10)
